...
class TistoryPoster:
    ...
    def _set_image_alt_text(self, alt_text: str):
        ...
        self._log("  → 인라인 이미지 툴바에서 대체 텍스트 버튼 클릭")
        time.sleep(1.0)

        toolbar = page.locator(".mce-floatpanel.mce-tinymce-inline, .mce-tinymce-inline.mce-floatpanel")
        if toolbar.count() == 0:
            self._log("  → 툴바 없음 — 이미지 클릭")
            page.locator(".mce-content-body img, #tinymce img, .mce-edit-area img").first.click(timeout=5_000)
            time.sleep(0.8)
            toolbar = page.locator(".mce-floatpanel.mce-tinymce-inline, .mce-tinymce-inline.mce-floatpanel")
            toolbar.first.wait_for(state="visible", timeout=8_000)

        alt_btn = toolbar.locator('[aria-label="대체 텍스트 삽입"]').first
        if alt_btn.count() == 0:
            alt_btn = page.locator(".mce-floatpanel.mce-tinymce-inline .mce-i-alt").first.locator(
                "xpath=ancestor::*[@role='button'][1]"
            )
        alt_btn.wait_for(state="visible", timeout=8_000)
        alt_btn.locator("button").first.click(timeout=5_000)
        self._log("  → 대체 텍스트 삽입 버튼 클릭 완료")

        alt_panel = page.locator(".keditor-image-alt-panel, .keditor-link-panel.keditor-image-alt-panel")
        alt_textarea = page.locator("textarea.kimage-alt, textarea.form-text.kimage-alt")
        alt_submit = page.locator("#kimage-alt-submit, .keditor-image-alt-panel button[type='submit']")

        for _ in range(25):
            if alt_panel.first.is_visible() or alt_textarea.first.is_visible():
                break
            time.sleep(0.25)
        else:
            raise RuntimeError("대체 텍스트 입력창(.keditor-image-alt-panel)이 열리지 않았습니다.")

        alt_textarea.first.wait_for(state="visible", timeout=5_000)
        alt_textarea.first.click(timeout=3_000)
        alt_textarea.first.fill(alt_text, timeout=5_000)
        self._log(f"  → 대체 텍스트 입력: {alt_text}")
        time.sleep(0.2)

        if alt_submit.count() > 0:
            alt_submit.first.click(timeout=5_000)
        else:
            page.locator(".keditor-image-alt-panel button:has-text('확인')").first.click(timeout=5_000)

        self._log("  → 대체 텍스트 확인 클릭 완료")
        time.sleep(0.8)
